...
from backend import handle_input_text, get_available_models
...
# Get Model command
async def get_model_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if (authenticate_user(update.message.from_user.id)):
        models = get_available_models()
        # Print in a nice format
        model_list = "\n".join(models)
        await update.message.reply_text(f'Available models are: \n{model_list}')
    else:
...
def authenticate_user(user_id: int) -> bool:
    return user_id in whitelisted_telegram_id
...
def main():
    ...
    app.add_handler(CommandHandler('getmodel', get_model_command))
